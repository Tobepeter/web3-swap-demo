...
describe('MockERC20', function () {
  ...
  describe('部署', function () {
    it('应该设置正确的名称和符号', async function () {
      expect(await mockToken.name()).to.equal('MockERC20 Token')
      expect(await mockToken.symbol()).to.equal('MockERC20')
    })
  ...
  describe('转账', function () {
    ...
    it('应该在余额不足时回滚', async function () {
      ...
      await expect(mockToken.connect(user).transfer(owner.address, transferAmount)).to.be.revertedWithCustomError(mockToken, 'ERC20InsufficientBalance')
    })
  })
})
