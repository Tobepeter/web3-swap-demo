...
import { liqControl } from '../LiqControl'
import { defaultState, store } from '@/store/store'
import type { PartialStoreState, StoreState } from '@/store/store'
...
// 创建 mock state
const createMockState = (overrides: PartialStoreState = {}): StoreState => Object.assign({ ...defaultState }, overrides)
...
describe('Liquidity Calculator', () => {
  ...
  describe('getUserLpPercent', () => {
    describe('当用户没有流动性时', () => {
      it('应该返回 0', () => {
        ...
        expect(liqControl.getUserLpPercent()).toBe(0)
      })
    })
  ...
  describe('getRemoveLiquidityTokens', () => {
    describe('当总流动性为 0 时', () => {
      it('应该返回 0', () => {
        ...
        const { amount0, amount1 } = liqControl.getRemoveLiquidityTokens(10)
    ...
    describe('当总流动性不为 0 时', () => {
      ...
      it('应该正确计算小数百分比（1.23%）', () => {
        ...
        const { amount0, amount1 } = liqControl.getRemoveLiquidityTokens(1.23)
      ...
      it('应该正确处理大数字', () => {
        ...
        const { amount0, amount1 } = liqControl.getRemoveLiquidityTokens(12.5)
  ...
  describe('getMaxAddLiquidityTokens', () => {
    it('应该根据当前池子比例计算最大可添加数量', () => {
      ...
      const { amount0, amount1 } = liqControl.getMaxAddLiquidityTokens()
